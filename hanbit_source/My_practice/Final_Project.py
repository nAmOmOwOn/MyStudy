...
def func_zoomin():
    global root, canvas, inPhoto, outPhoto, inX, inY
    scale = askinteger("확대 배수", "확대할 배수를 입력하세요", minvalue=2,maxvalue=8)
    outPhoto = inPhoto.copy()
...
def func_zoomout():
    global root, canvas, inPhoto, outPhoto, inX, inY
    scale = askinteger("축소 배수", "축소할 배수를 입력하세요", minvalue=2,maxvalue=8)
    outPhoto = inPhoto.copy()
    outPhoto = outPhoto.resize((int(inX / scale), int(inY / scale)))
    outX = outPhoto.width
    outY = outPhoto.height
...
mainMenu.add_cascade(label = "이미지처리(2)", menu = effect2Menu)
...
effect2Menu.add_command(label = "경계선 추출", command = func_contour)
root.mainloop()
