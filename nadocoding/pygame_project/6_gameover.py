...
balls.append({
    ...
    "to_x" : 3, # x축 이동 방향 -3 이면 왼쪽, 3 이면 오른쪽으로 이동
    "to_y" : -6, # y축 이동 방향
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        else: # 그 외의 모든 경우에는 속도를 증가(시작값이 - 이므로 감소효과)
            ball_val["to_y"] += 0.5
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3:
                    ...
                    # 왼쪽으로 튕겨나가는 작은 공
                    balls.append({
                        "pos_x" : ball_pos_x + (ball_widht / 2) - (small_ball_width / 2), # 공의 x 좌표
